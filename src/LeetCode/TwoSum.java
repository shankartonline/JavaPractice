...
public class TwoSum {
    public static void main(String[] args) {
        int[] nums1 = {3, 4, 5, 6};
        int target1 = 7;
        System.out.println(isTwoSum(nums1, target1));
    }

    private static List<Integer> isTwoSum(int[] nums, int target) {
        Map<Integer, Integer> numMap = new HashMap<>();

        for (int i = 0; i < nums.length; i++) {
            int complement = target - nums[i];

            // Check if we've already seen the needed complement
        ...
        return null; // Per problem constraints, this line is never reached
    }
}
